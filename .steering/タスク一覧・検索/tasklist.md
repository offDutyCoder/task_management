...
**技術的な学び**:
- EF Core の LINQ Where チェーンは `HasValue` チェックと組み合わせることで nullable な DateTime フィルターを安全に実装できる。`t.DueDate.HasValue && t.DueDate.Value >= filter.DueAfter.Value` のパターンが有効。
- InMemory EF Core を使ったリポジトリ直接テストは Service ユニットテスト（Moq使用）よりもリポジトリのクエリロジックを確実に検証できる。
- Angular の `MatCheckboxModule` は `ReactiveFormsModule` と組み合わせる際、`formControlName` を使うことでチェックボックスの on/off を boolean として `formGroup` に自動バインドできる。
- HTML `<input type="date">` は `yyyy-MM-dd` 形式の文字列を返すため、ASP.NET Core の `DateTime?` バインディングと自然に互換する。

**プロセス上の改善点**:
- 既存の実装パターン（前フェーズの steering ドキュメント）を事前に確認することで、新規ファイル作成なしに最小変更で機能追加できた。
- バックエンド → フロントエンドの順で実装することで、型不一致を早期に発見できる。

### 次回への改善提案
- ソート機能（列ヘッダークリックによる `sortBy`/`sortOrder` パラメーター）を追加する際は、`TaskFilterQuery` に同パターンで追加し、Repository の `OrderByDescending` を条件分岐に変更する。
- フィルター条件を URL クエリパラメーターに保存する機能（ブックマーク可能なフィルター）は Angular の `ActivatedRoute.queryParams` を使って実装できる。
- `data-testid` 属性を期限範囲入力と isRecurring チェックボックスに付与すれば E2E テストが書きやすくなる。
